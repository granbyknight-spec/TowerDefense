...
class TitleScene extends Phaser.Scene {
  ...
  preload() {
    ...
    // Chibi sprites for hero showcase (graceful — falls back to emoji if missing)
    this.load.on('loaderror', () => {});
    ['PUPPY_KNIGHT', 'CORGI_HEALER', 'LABRADOR_SCOUT', 'BEAGLE_ARCHER',
     'POODLE_MAGE', 'BULLDOG_TANK', 'HUSKY_RIDER', 'TERRIER_THIEF',
     'FOX_SCOUT', 'OTTER_ALLY', 'DOG_PALADIN'].forEach(id => {
      this.load.image('player_png_' + id, 'assets/characters/' + id.toLowerCase() + '_chibi_v1.png');
    });
  }
  ...
  create() {
    ...
    // ── Chibi showcase ────────────────────────────────────────────────────────
    // Show whichever hero chibi PNGs are actually loaded, up to 11 slots
    const portraitIds = ['PUPPY_KNIGHT','CORGI_HEALER','LABRADOR_SCOUT','BEAGLE_ARCHER',
                         'POODLE_MAGE','BULLDOG_TANK','HUSKY_RIDER','TERRIER_THIEF',
                         'FOX_SCOUT','OTTER_ALLY','DOG_PALADIN'];
    const portraitColors = {
      PUPPY_KNIGHT:   0xf8d030, CORGI_HEALER:   0x33bb55, LABRADOR_SCOUT: 0xff8800,
      BEAGLE_ARCHER:  0x44ccaa, POODLE_MAGE:    0xaa44ff, BULLDOG_TANK:   0xcc4444,
      HUSKY_RIDER:    0x4488ff, TERRIER_THIEF:  0x888800, FOX_SCOUT:      0xff6622,
      OTTER_ALLY:     0x22aacc, DOG_PALADIN:    0xffffff,
    };
